...
gpgauth.utils = {
    ...
    /*
        Function: init
            Initializes the gpgauth.utils object and setups up required overrides.
    */
    init: function() {
        if (this.detectedBrowser['vendor'] == "mozilla") {
            if (gpgauth.constants.debug.LOG) {
    ...
    resourcePath: function() {
        ...
        if (userAgent.search("firefox") > -1 ||
          ...
          userAgent.search("seamonkey") > -1)
            return "chrome://gpgauth-firefox/content/";   
...
//                }
    ...
    */
    openNewTab: function(url, tabIndex) {
        var _ = gpgauth.utils.i18n.gettext;
        switch (gpgauth.utils.detectedBrowser['product']) {
            case "firefox":
            case "thunderbird":
            case "conkeror":
            case "seamonkey":
                ...
                wTitle = (url.search("options_tab=0") > -1) ? _("gpgauth Options") :
                    (url.search("options_tab=1") > -1) ? _("gpgauth Key Manager") :
                    (url.search("options_tab=2") > -1) ? _("About gpgauth") : "";
    ...
    */
    getSelectedText: function(context) {
        ...
        if (!selectionObject.toString().length > 0 && selectionTarget.nodeName == "IFRAME" &&
            selectionTarget.className.indexOf("gpgauth-result-frame") < 0 &&
            selectionTarget.className.indexOf("gpgauth-dialog") < 0) {
        ...
        if (selectionObject && selectionObject.rangeCount > 0) {
            gpgauth.overlay.insert_range = selectionObject.getRangeAt(0);
            if (gpgauth.overlay.insert_range.startOffset == gpgauth.overlay.insert_range.endOffset)
                gpgauth.overlay.insert_range = null;
        } else {
            gpgauth.overlay.insert_range = null;
        }
        ...
        if (selectionTarget)
            gpgauth.overlay.insert_target = selectionTarget;
    ...
    */
    ...
    /*
        Class:  gpgauth.utils._onRequest
    ...
    _onRequest: {
        ...
        */
        addListener: function(callback) { // analogue of chrome.extension.onRequest.addListener
            if (gpgauth.utils.detectedBrowser['product'] == "safari" ||
               (gpgauth.utils.detectedBrowser['vendor'] == "mozilla" &&
               gpgauth.utils.detectedBrowser['product'] != "thunderbird")) {
                return document.addEventListener("listener-query", function(event) {
                    var node = event.target, doc = node.ownerDocument;

                    var userData = (gpgauth.utils.detectedBrowser['vendor'] == "mozilla") ?
                        node.getUserData("data") : jq(node).data("data");

                    var userCallback = (gpgauth.utils.detectedBrowser['vendor'] == "mozilla") ?
                        node.getUserData("callback") : jq(node).data("callback");

                    if (gpgauth.utils.detectedBrowser['vendor'] != "mozilla" &&  !userData)
                        userData = event.detail.data;

                    if (gpgauth.utils.detectedBrowser['vendor'] != "mozilla" && !userCallback)
    ...
    */
    tabs: {
        ...
        */
        sendRequest: function(target, request) {
            if (gpgauth.utils.detectedBrowser['vendor'] == "mozilla" ||
                gpgauth.utils.detectedBrowser['product'] == "safari") {
                if (!request.target_id) {
                    gpgauth.utils.sendRequest(request);
                } else {
                    var iframe = gpgauth.utils.getFrameById(request.target_id);
                    if (iframe) {
    ...
    /*
        Class: gpgauth.utils.contextMenus
    ...
    contextMenus: {
        ...
        */
        ...
        /*
            ...
            Parameters:
                action - <int> The gpgauth.constants.overlayAction to add
        */
        add: function(action) {
            var _ = gpgauth.utils.i18n.gettext;
            switch (action) {
                case gpgauth.constants.overlayActions.EXPORT:
                    if (gpgauth.utils.detectedBrowser['vendor'] == "mozilla") {
                        jq(".gpgauth-menu-export")[0].hidden = false;
                    } else if (gpgauth.utils.detectedBrowser['product'] == "chrome") {
                        var id = "gpgauth-context-insert-pubkey";
                        chrome.contextMenus.create({
                            "title" : _("Paste Public Key"),
                            "contexts" : ["editable", "frame", "selection"],
                            "type" : "normal",
                            "onclick" : function(info, tab) {
                                gpgauth.utils.tabs.sendRequest(tab, {
                ...
                case gpgauth.constants.overlayActions.PSIGN:
                    if (gpgauth.utils.detectedBrowser['vendor'] == "mozilla") {
                        jq(".gpgauth-menu-sign")[0].hidden = false;
                    } else if (gpgauth.utils.detectedBrowser['product'] == "chrome") {
                        var id = "gpgauth-context-clearsign";
                        chrome.contextMenus.create({
                ...
                case gpgauth.constants.overlayActions.IMPORT:
                    if (gpgauth.utils.detectedBrowser['vendor'] == "mozilla") {
                        jq(".gpgauth-menu-import")[0].hidden = false;
                    } else if (gpgauth.utils.detectedBrowser['product'] == "chrome") {
                        var id = "gpgauth-context-import";
                        chrome.contextMenus.create({
                ...
                case gpgauth.constants.overlayActions.CRYPT:
                    if (gpgauth.utils.detectedBrowser['vendor'] == "mozilla") {
                        jq(".gpgauth-menu-crypt")[0].hidden = false;
                    } else if (gpgauth.utils.detectedBrowser['product'] == "chrome") {
                        var id = "gpgauth-context-encrypt";
                        chrome.contextMenus.create({
                ...
                case gpgauth.constants.overlayActions.DECRYPT:
                    if (gpgauth.utils.detectedBrowser['vendor'] == "mozilla") {
                        jq(".gpgauth-menu-decrypt")[0].hidden = false;
                    } else if (gpgauth.utils.detectedBrowser['product'] == "chrome") {
                        var id = "gpgauth-context-decrypt";
                        chrome.contextMenus.create({
                ...
                case gpgauth.constants.overlayActions.VERIF:
                    if (gpgauth.utils.detectedBrowser['vendor'] == "mozilla") {
                        jq(".gpgauth-menu-verif")[0].hidden = false;
                    } else if (gpgauth.utils.detectedBrowser['product'] == "chrome") {
                        var id = "gpgauth-context-verify";
                        chrome.contextMenus.create({
                ...
                case gpgauth.constants.overlayActions.OPTS:
                    if (gpgauth.utils.detectedBrowser['vendor'] == "mozilla") {
                        jq(".gpgauth-menu-options")[0].hidden = false;
                    } else if (gpgauth.utils.detectedBrowser['product'] == "chrome") {
                        var id = "gpgauth-context-separator";
                        chrome.contextMenus.create({
                            "type" : "separator",
                            "contexts" : ["all", "page"],
                            "onclick" : function(info, tab) {
                        ...
                        var id = "gpgauth-context-options";
                        chrome.contextMenus.create({
                            "title" : _("Options"),
                            "type" : "normal",
                            "contexts" : ["all", "page"],
                            "onclick" : function(info, tab) {
                                ...
                                if (gpgauth.utils.detectedBrowser['product'] == "chrome")
                                    url += "?auto_init=true"
                                gpgauth.utils.openNewTab(gpgauth.utils.resourcePath + url);
                            },
                        });
                    }
                    break;
                    
                case gpgauth.constants.overlayActions.MANAGER:
                    if (gpgauth.utils.detectedBrowser['vendor'] == "mozilla") {
                        jq(".gpgauth-menu-manager")[0].hidden = false;
                    } else if (gpgauth.utils.detectedBrowser['product'] == "chrome") {
                        var id = "gpgauth-context-manager";
                        chrome.contextMenus.create({
                            "title" : _("Key Manager"),
                            "type" : "normal",
                            "contexts" : ["all", "page"],
        ...
        }, // end gpgauth.utils.contextMenus.add
    },
    ...
    i18n: {
        gettext: function(msg) {
            if (gpgauth.utils.detectedBrowser['vendor'] == "mozilla") {
                ...
                var stringBundle = mainWindow.document.getElementById("gpgauth-strings");
...
gpgauth.descript = function(html) { return html.replace(/\<script(.|\n)*?\>(.|\n)*?\<\/script\>/g, "") };
