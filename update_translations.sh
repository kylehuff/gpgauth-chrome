...
# Creates required locale files for the gpgauth extension project (firefox/chrome)
# Requirements:
#   pybabel
#   gettext

BUGSADDR=gpgauth-translations@gpgauth.org
...
pybabel extract -F gpgauth-babel.cfg --copyright-holder=$ORGANIZATION --msgid-bugs-address=$BUGSADDR --project=$PROJECT --version=$VERSION $SRCDIR > $PODIR/$POTFILE
...
if ${FFEXT:=true}; then
    ...
    do
        if [ -f $PODIR/$locale.po ]; then
            ...
            echo "Creating $LOCALEDIR/$locale/gpgauth.dtd"
            gawk 'BEGIN{ FS="\n"; RS="";} match($0, /msgid.*(\"(.*)\")$/, b) && match(b[0], /msgid.*(\"(.*)\")(\n)?msgstr.*(\".*\")(\n)?/, a) { if (a[2]!="") printf("<!ENTITY %s %s>\n", gensub(/[^\"|^a-Z|^0-9|^\.]/, "_", "g", a[2]), a[4]) }' $PODIR/$locale.po > $LOCALEDIR/$locale/gpgauth.dtd
            echo "Creating $LOCALEDIR/$locale/gpgauth.properties"
            gawk 'BEGIN{ FS="\n"; RS="";} match($0, /msgid.*(\"(.*)\")$/, b) && match(b[0], /msgid.*(\"(.*)\")(\n)?msgstr.*(\"(.*)\")(\n)?/, a) { if (a[2]!="") printf("%s=%s\n", gensub(/[^\"|^a-Z|^0-9|^\.]/, "_", "g", a[2]), a[5]) }' $PODIR/$locale.po > $LOCALEDIR/$locale/gpgauth.properties
        fi
    done
else
